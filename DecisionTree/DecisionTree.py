...
def entropy(rows):
    ...
    for lbl in counts:
        prob_of_lbl = counts[lbl] / float(len(rows))
        lbl_entropy = -(prob_of_lbl * math.log(prob_of_lbl, 2))
        entropy=entropy+lbl_entropy
    return entropy
...
def print_tree(node, depth=0,spacing=""):
    ...
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        print(spacing + "Leaf with node id " + str(node.id) + " Predict " + str(node.predictions) + " Class = " + str(node.predictedLabel) )
